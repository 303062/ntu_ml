...
def sce(
    ...
    if not task_tensors:
        raise ValueError("task_tensors must not be empty")
    
    ## Stack all task tensors into a single tensor of shape [num_tasks, ...]
    task_tensor_stack = torch.stack(task_tensors, dim=0)
    mask_dtype = task_tensor_stack.dtype
    ...
    if density < 1.0:
        sparse_mask = sce_mask(task_tensor_stack, density, mask_dtype)
        task_tensor_stack = task_tensor_stack * sparse_mask.unsqueeze(0)
    
    ## Compute a binary mask indicating majority sign agreement per element across task vectors. (reuse)
    erase_mask = calculate_majority_sign_mask(task_tensor_stack, method=majority_sign_method)
    
    ## Compute task-specific weights with sce_weight()
    task_weights = sce_weight(task_tensor_stack)
    
    ## Reshape weights to match dimensions of task_tensors for broadcasting.
    while task_weights.dim() < task_tensor_stack.dim():
    ...
    masked_weights = task_weights * erase_mask
    
    ## Weighted summation of masked task tensors to create a merged task tensor.
    merged_tensor = (task_tensor_stack * masked_weights).sum(dim=0)
    ...
    normalizer = torch.sum(masked_weights, dim=0).clamp(min=1e-6)
    final_tensor = merged_tensor / normalizer
